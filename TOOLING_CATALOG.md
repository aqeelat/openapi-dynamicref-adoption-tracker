...
| **Scalar** | https://github.com/scalar/scalar | MIT (OSS) | No support | Modern API documentation renderer + API client. `@scalar/json-magic` bundler only checks `$ref` (`hasRef()`), `SchemaObject` lacks `$dynamicRef`/`$dynamicAnchor`/`$id`/`$anchor` fields. Maintainers aware of `$dynamicRef` from PR #8359. Issue #9414 open. High landing likelihood. [Full analysis](analysis/scalar.md) |
